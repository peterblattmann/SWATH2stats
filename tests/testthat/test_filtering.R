...
test_that("filtering", {
  ...
  expect_that(filter_mscore_requant(data, 0.01, 0.8), shows_message("Treshold, peptides need to have been quantified in more conditions than: 4.8"))
  expect_that(filter_mscore_requant(data, 0.01, 0.8), shows_message("Fraction of peptides selected: 0.42"))
  expect_that(filter_mscore_requant(data, 0.01, 0.8), shows_message("Dimension difference: 1323, 0"))
  ...
  # filter on_fdr
  expect_that(filter_mscore_fdr(data.FDR, FFT = 0.7, overall_protein_fdr_target=0.1, upper_overall_peptide_fdr_limit=0.05), 
