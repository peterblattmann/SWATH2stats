assess_fdr_overall <-function(data, FFT=1, n.range=20, output="pdf_csv",
  ...
  for (i in seq_len(length(mscore_levels))) {
    target.assays[i] <- length(unique(data[data[["decoy"]] == FALSE &
                                           data[["m_score"]] <= mscore_levels[i],
                                           c("transition_group_id")]))
    decoy.assays[i] <- length(unique(data[data[["decoy"]] == TRUE &
                                          data[["m_score"]] <= mscore_levels[i],
                                          c("transition_group_id")]))
    target.peptides[i] <- length(unique(data[data[["decoy"]] == FALSE &
                                             data[["m_score"]] <= mscore_levels[i],
                                             c("fullpeptidename")]))
    decoy.peptides[i] <- length(unique(data[data[["decoy"]] == TRUE &
                                            data[["m_score"]] <= mscore_levels[i],
                                            c("fullpeptidename")]))
    target.proteins[i] <- length(unique(data[data[["decoy"]] == FALSE &
                                             data[["m_score"]] <= mscore_levels[i],
                                             c("proteinname")]))
    decoy.proteins[i] <- length(unique(data[data[["decoy"]] == TRUE &
                                            data[["m_score"]] <= mscore_levels[i],
                                            c("proteinname")]))
  }
  ...
  if (output == "pdf_csv") {
    fdr_table_csv <- cbind(mscore_levels, target.assays, decoy.assays,
                           assay.fdr, true.target.assays, target.peptides,
                           decoy.peptides, peptide.fdr, true.target.peptides,
