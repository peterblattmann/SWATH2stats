...
assess_fdr_byrun <- function(data, FFT = 1, n.range = 20, output = "pdf_csv", plot = TRUE,
    ...
    for (i in seq_len(n.range)) {
        decoy_count_lengths[i] <- length(by(data[data$decoy == TRUE & data[, mscore.col] <=
            test_levels[i], c("transition_group_id")], data[data$decoy == TRUE &
            data[, mscore.col] <= test_levels[i], c("run_id")], length))
    }
    ...
    for (i in seq_len(length(mscore_levels))) {
        ...
        fdr_cube[10, , i] <- by(data.d[data.d[, mscore.col] <= mscore_levels[i],
            c("ProteinName")], data.d[data.d[, mscore.col] <= mscore_levels[i], c("run_id")],
            length_unique)
    ...
    message("The average FDR by run on assay level is ", round(mean(fdr_cube[4, ,
        1], na.rm = TRUE), digits = 3), "\n")
    message("The average FDR by run on peptide level is ", round(mean(fdr_cube[8,
        , 1], na.rm = TRUE), digits = 3), "\n")
    message("The average FDR by run on protein level is ", round(mean(fdr_cube[12,
        , 1], na.rm = TRUE), digits = 3), "\n")

    if (output == "pdf_csv") {
        message("Individual run FDR qualities can be retrieved from ", paste(filename,
            ".csv"), "\n", sep = "")
