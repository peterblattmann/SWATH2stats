assess_fdr_byrun <- function(data, FFT = 1, output = "pdf_csv", plot = TRUE, filename = "FDR_report_byrun") 
{
  ...
  for (i in seq(1:.n.intervall)) {
    decoy_count_lengths[i] <- length(by(data[data$decoy == TRUE & data$m_score <= test_levels[i],
        c("id")], data[data$decoy == TRUE & data$m_score <= test_levels[i], c("run_id")], length))
  }
  ...
  for (i in 1:length(mscore_levels)) {
    ...
    fdr_cube[1, , i] <- by(data.t[data.t$m_score <= mscore_levels[i], c("id")],
                           data.t[data.t$m_score <= mscore_levels[i],c("run_id")], length)
    fdr_cube[2, , i] <- by(data.d[data.d$m_score <= mscore_levels[i], c("id")],
