...
#' @export
plot_variation_vs_total <- function(data, column.values = "Intensity", Comparison1 = transition_group_id ~
    BioReplicate + Condition, Comparison2 = transition_group_id + Condition ~ BioReplicate,
    fun.aggregate = NULL, label = FALSE, title = "coefficient of variation - total versus within replicates",
    boxplot = TRUE, ...) {
    ...
    data2.mean <- apply(data2.c[, n_vars2:dim(data2.c)[2]], 1, function(x) mean(x,
        na.rm = TRUE))
