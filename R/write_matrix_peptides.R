write_matrix_peptides <- function(data, write.csv = FALSE, filename = "SWATH2stats_overview_matrix_peptidelevel.csv", 
    rm.decoy = FALSE) {
    if (rm.decoy == TRUE) {
        data <- subset(data, data$decoy == 0)
    }
    data.peptide <- data[, c("ProteinName", "run_id", "FullPeptideName", "Intensity")]
    ProteinName_FullPeptideName <- paste(data.peptide$ProteinName, data.peptide$FullPeptideName, 
        sep = "_")
