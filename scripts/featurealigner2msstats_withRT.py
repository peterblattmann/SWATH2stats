...
out = open(outarg, 'w')
out.write(
    "ProteinName,PeptideSequence,PrecursorCharge,FragmentIon,ProductCharge,IsotopeLabelType,Intensity,RT,Condition,BioReplicate,Run\n")
...
for line in csvf:
    ...
    for fa, pa in zip(fas, pas):
        out.write("%s,%s,%s,%s,NA,light,%s,%s,%s,%s,%s\n" % (line["ProteinName"], line["FullPeptideName"], line["Charge"], fa, pa, line.get("RT",""), line.get("Condition",""), line.get("BioReplicate",""), name))
